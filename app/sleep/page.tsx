...
export default function SleepPage() {
  ...
  useEffect(() => {
    ...
    ]).then(([planData, logsData, streakData, profile]) => {
      if (!planData.redirect) setSleepPlan(planData);
  ...
  function getRowState(dateStr: string): RowState {
    return rowStates[dateStr] ?? {
      mode: "view",
      hitTarget: null,
      actualBedtime: minutesToTime(parseTimeMin(userBedtime) + 30),
      actualWakeTime: userWake,
      saving: false,
    };
  }
  ...
  const plan = sleepPlan?.plan;
  const todayPlan = plan?.[0];
  const circadian = todayPlan?.circadian ?? null;
  ...
  return (
    <div className="min-h-screen bg-white dark:bg-[#1a1a1a]">
      ...
      {todayPlan?.windDown && (
        <section className="border-b border-[#E5E5E5] dark:border-[#333] px-6 py-10">
          <div className="max-w-[1200px] mx-auto">
            ...
            <FadeUp delay={80}>
              <WindDownTimeline windDown={todayPlan.windDown} />
            </FadeUp>
          </div>
        </section>
      )}
      ...
      {/* Circadian Protocol */}
      {circadian && <CircadianProtocolSection circadian={circadian} />}
      ...
      <div className="max-w-[1200px] mx-auto px-6 pb-10">
        <a
          href="/analysis"
          className="inline-block border border-[#0A0A0A] dark:border-[#F5F5F5] text-[#0A0A0A] dark:text-[#F5F5F5] font-black text-xs uppercase tracking-widest px-6 py-3 hover:bg-[#0A0A0A] dark:hover:bg-[#F5F5F5] hover:text-white dark:hover:text-[#0A0A0A] transition-colors"
        >
          VIEW FULL ANALYSIS →
        </a>
      </div>
